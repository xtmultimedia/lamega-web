/* global React, Icon, Section, Bloom, Button, PLAYLISTS, SOCIAL */
// La Mega 99.9 — Spotify playlists carousel
...
function PlaylistCard({ pl }) {
  ...
  return (
    <div className="glass" style={{
      ...
      {/* cover */}
      <div style={{ position: 'relative', aspectRatio: '1', borderRadius: 'var(--r-sm)', overflow: 'hidden', marginBottom: 16, background: `linear-gradient(135deg, ${pl.hue1}, ${pl.hue2})`, boxShadow: 'inset 0 0 40px rgba(0,0,0,0.35)' }}>
        ...
        <span className="display" style={{ position: 'absolute', left: 14, bottom: 12, right: 14, fontSize: 24, color: '#fff', lineHeight: 0.95, textShadow: '0 2px 10px rgba(0,0,0,0.4)' }}>{pl.name}</span>
        ...
        {/* play fab */}
        <div style={{ position: 'absolute', right: 12, bottom: 12, width: 46, height: 46, borderRadius: '50%', background: 'var(--green)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 6px 16px rgba(0,0,0,0.4)', transform: h ? 'translateY(0) scale(1)' : 'translateY(8px) scale(0.9)', opacity: h ? 1 : 0, transition: 'all var(--dur) var(--ease-out)' }}>
...
function Playlists() {
  ...
  const scroll = (dir) => { scroller.current?.scrollBy({ left: dir * 500, behavior: 'smooth' }); };
  return (
    <Section id="playlists" style={{ background: 'var(--bg-1)', borderTop: '1px solid var(--line-1)', borderBottom: '1px solid var(--line-1)' }}>
      ...
      <div className="reveal" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24, marginBottom: 56, flexWrap: 'wrap' }}>
        <div>
          ...
          <h2 className="h2">Nuestras <span style={{ color: 'var(--green)' }}>Playlists</span></h2>
          <p className="lead" style={{ maxWidth: 460, marginTop: 18 }}>Curadas por nuestros locutores. Dale play y llévate La Mega contigo.</p>
        </div>
      ...
      <div ref={scroller} className="pl-scroller" style={{ display: 'flex', gap: 18, overflowX: 'auto', scrollSnapType: 'x mandatory', margin: '0 -48px', padding: '0 48px 14px' }}>
        {PLAYLISTS.map((pl) => <PlaylistCard key={pl.name} pl={pl} />)}
      </div>
      <style>{`
...
function ArrowBtn({ dir, onClick }) {
  ...
  const [h, setH] = useState(false);
  return (
    <button onClick={onClick} aria-label={dir} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ width: 46, height: 46, borderRadius: '50%', border: '1px solid var(--line-2)', background: h ? 'var(--bg-3)' : 'var(--bg-2)', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all var(--dur)' }}>
