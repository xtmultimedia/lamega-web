/* global React, Icon, Section, Bloom, SocialIcon, SOCIAL */
// La Mega 99.9 — Social / live feed (3-column)

function FeedShell({ platform, color, handle, followers, children }) {
  ...
  return (
    <div className="reveal glass" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 18px', borderBottom: '1px solid var(--line-1)' }}>
        <span style={{ width: 40, height: 40, borderRadius: '50%', background: `${color}22`, border: `1px solid ${color}66`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
...
function Social() {
  ...
  return (
    <Section id="social" style={{ background: 'var(--bg-1)', borderTop: '1px solid var(--line-1)' }}>
      ...
      <SectionHead align="center" index="05" kicker="NACIÓN MEGA"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 22 }} className="social-grid">
        ...
        <FeedShell platform="instagram" color={ig} handle="@lamega99.9ec" followers="284K">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 3, padding: 3 }}>
            {[['#E31E24', '#7A1FC4'], ['#1683C8', '#E31E24'], ['#D0307A', '#FF2D34'], ['#FF2D34', '#8E0F13'], ['#7A1FC4', '#1683C8'], ['#E31E24', '#D0307A']].map((g, i) => (
        ...
        <FeedShell platform="tiktok" color={tt} handle="@lamega99.9" followers="512K">
          <div style={{ position: 'relative', aspectRatio: '3/4', background: 'linear-gradient(160deg, #25F4EE22, #FE2C5522 60%, #0a0a0a)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: 3, borderRadius: 'var(--r-sm)' }}>
            <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 50% 40%, rgba(255,255,255,0.10), transparent 55%)' }} />
            <span style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(4px)', border: '1px solid rgba(255,255,255,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ position: 'absolute', right: 12, bottom: 16, display: 'flex', flexDirection: 'column', gap: 16, alignItems: 'center', color: '#fff' }}>
              <span style={{ textAlign: 'center' }}><Icon name="heart" size={24} /><span className="mono" style={{ display: 'block', fontSize: 10 }}>48K</span></span>
              <span style={{ textAlign: 'center' }}><Icon name="message-circle" size={24} /><span className="mono" style={{ display: 'block', fontSize: 10 }}>1.2K</span></span>
              <span style={{ textAlign: 'center' }}><Icon name="share-2" size={24} /><span className="mono" style={{ display: 'block', fontSize: 10 }}>3.4K</span></span>
            </div>
            <div className="mono" style={{ position: 'absolute', left: 12, bottom: 14, fontSize: 11, color: '#fff' }}>♪ Detrás de cámaras · El Ganado</div>
          </div>
        </FeedShell>

        {/* Facebook Live */}
        <FeedShell platform="facebook" color={fb} handle="La Mega 99.9 Ecuador" followers="631K">
          <div style={{ position: 'relative', aspectRatio: '16/10', background: 'radial-gradient(120% 120% at 50% 0%, #16213e, #0a0a0a)', margin: 3, borderRadius: 'var(--r-sm)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ position: 'absolute', top: 10, left: 10, display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--red)', color: '#fff', fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 10, letterSpacing: '0.1em', padding: '4px 9px', borderRadius: 4 }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#fff', animation: 'pulse-dot 1.3s infinite' }} />LIVE
            </div>
            <span style={{ width: 58, height: 58, borderRadius: '50%', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Icon name="play" size={26} color="#fff" strokeWidth={2.2} style={{ marginLeft: 2 }} />
            </span>
            <div className="mono" style={{ position: 'absolute', right: 10, bottom: 10, fontSize: 10, color: '#fff', background: 'rgba(0,0,0,0.5)', padding: '3px 8px', borderRadius: 4 }}>● 892 viendo</div>
          </div>
          <div style={{ padding: '12px 16px 16px' }}>
            <div style={{ fontSize: 13.5, color: 'var(--fg-1)', lineHeight: 1.4 }}>Transmitiendo en vivo: <strong style={{ color: '#fff' }}>El Ganado</strong> con sorteo de entradas 🎟️</div>
          </div>
        </FeedShell>
      </div>
