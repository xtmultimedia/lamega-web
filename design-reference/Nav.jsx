/* global React, Icon, OnAir, SocialIcon, NAV_LINKS, usePlayer */
// La Mega 99.9 — sticky glass navbar + full-screen mobile overlay
    ...
    <header style={{
      ...
      <div className="nav-inner" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0, zIndex: 2 }}>
          <a href="Formulario.html" className="nav-cta" style={{
          ...
          <a href="Dashboard.html" className="nav-admin" aria-label="Panel admin" style={{
          ...
          }}><Icon name="layout-dashboard" size={17} /></a>
          <button className="nav-burger" aria-label="Menú" onClick={() => setOpen((o) => !o)}
            style={{ display: 'none', background: 'var(--bg-2)', border: '1px solid var(--line-2)', borderRadius: 'var(--r-sm)', width: 44, height: 44, color: '#fff', cursor: 'pointer', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div className="nav-overlay" aria-hidden={!open} style={{
        ...
        <div style={{ marginBottom: 28 }}><OnAir /></div>
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {NAV_LINKS.map((l, i) => (
            <a key={l.href} href={l.href} onClick={() => setOpen(false)} className="nav-ov-link"
              style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'clamp(30px, 9vw, 44px)', textTransform: 'uppercase', letterSpacing: '-0.01em', color: '#fff', padding: '8px 0', display: 'flex', alignItems: 'center', gap: 16 }}>
              <span className="mono" style={{ fontSize: 13, color: 'var(--red)', fontWeight: 700 }}>{String(i + 1).padStart(2, '0')}</span>
              {l.label}
            </a>
          ))}
          <a href="Formulario.html" className="nav-ov-link" style={{ marginTop: 18, fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'clamp(24px, 7vw, 34px)', textTransform: 'uppercase', color: 'var(--red-bright)', display: 'flex', alignItems: 'center', gap: 12 }}>
            <Icon name="music" size={26} /> Pide tu canción
          </a>
          <a href="Dashboard.html" className="nav-ov-link" style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 'clamp(20px, 6vw, 26px)', textTransform: 'uppercase', color: 'var(--fg-2)', display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
            <Icon name="layout-dashboard" size={22} /> Panel Admin
          </a>
        </nav>
        <div style={{ display: 'flex', gap: 12, marginTop: 40 }}>
          {['instagram', 'tiktok', 'facebook', 'youtube', 'spotify'].map((n) => <SocialIcon key={n} name={n} size={20} />)}
        </div>
      </div>
